<template>
  <div class="min-h-screen bg-[#f5f5f7]">
    <!-- Apple-like Header -->
    <header
      class="bg-white border-b border-gray-200 backdrop-blur-lg bg-opacity-80 sticky top-0 z-50"
    >
    ...
    <main class="max-w-[1440px] mx-auto px-8 py-12">
        ...
        <div class="flex items-center justify-between mb-8">
          <div>
            ...
            <p class="text-gray-500 mt-2 text-lg">
              Transform your JSON data into beautifully formatted HTML or
              Markdown
            </p>
          </div>
          <div class="flex items-center gap-3">
            <button
              class="px-4 py-2 text-sm text-gray-600 hover:text-gray-900 transition-colors rounded-full hover:bg-gray-50"
            >
      ...
      <div class="grid grid-cols-2 gap-6">
        ...
        >
          <div
            class="bg-gray-900 px-4 py-3 flex items-center justify-between border-b border-gray-800"
          >
...
<script setup>
  ...
  const convertToMarkdown = () => {
    ...
    try {
      ...
      state.value.renderedContent = sanitizeHtml(
        md.render(state.value.markdownContent),
      );
      state.value.statusMessage = '';
    } catch (error) {
      state.value.statusMessage = 'Invalid JSON data.';
  ...
  const jsonToHtml = (data, level = 0) => {
    ...
    if (typeof data === 'object' && data !== null) {
      return `<div class="${indentClass} p-4">${Object.entries(data)
        .map(
          ([key, value]) =>
            `<div class="mb-1"><strong>${key}:</strong> ${jsonToHtml(
              value,
              level + 1,
            )}</div>`,
        )
  ...
  const jsonToMarkdown = (data, level = 0) => {
    ...
    if (Array.isArray(data)) {
      return data
        .map((item) => `${indent}- ${jsonToMarkdown(item, level + 1)}`)
        .join('\n');
    }
